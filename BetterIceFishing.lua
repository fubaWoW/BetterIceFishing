...
function addon:DebugPrint(text)
  if internal.debug then
    DEFAULT_CHAT_FRAME:AddMessage("|cffff8000Debug\[|r"..text.."|cffff8000\]")
  end
end
...
function addon:CreateSettings()
  ...
  local info = {
    ...
    "useBetterFishingKey#Use Keybinding from \"Better Fishing\""
  }
  local prevCheckButton
  for i, keyInfo in ipairs(info) do
    local option, text = strsplit("#", keyInfo)
			if not (option == "useBetterFishingKey" and (not IsAddOnLoaded("BetterFishing"))) then
				...
				checkButton:SetChecked(BetterIceFishingDB[option])
				checkButton:SetScript("OnClick", function()
					BetterIceFishingDB[option] = not BetterIceFishingDB[option]
					checkButton:SetChecked(BetterIceFishingDB[option])
				end)
...
SlashCmdList[addonName .. 'Options'] = function()
